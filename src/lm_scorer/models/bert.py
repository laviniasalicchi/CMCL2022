...
class BERTLMScorer(TransformersLMScorer):
    ...
    def _build(self, model_name: str, options: Dict[str, Any]) -> None:
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
                model_name, use_fast=False, add_special_tokens=False
        )
    ...
    def _add_special_tokens(self, text: str) -> str:
        return self.tokenizer.cls_token + ' ' + text + ' ' + self.tokenizer.sep_token
    ...
    def _tokens_log_prob_for_batch(
        ...
        for sent_index in range(len(text)):
            ...
            sent_tokens = self.tokenizer.tokenize('[CLS] ' + text[sent_index])
            # sent_tokens = self.tokenizer.decode(ids.cpu().numpy().squeeze()).split()

            # sent_ids.shape = [len(text[sent_index]) + 1]
            sent_ids = ids[sent_index, sent_nopad_mask][0:-1]
    ...
    def _supported_model_names(cls) -> Iterable[str]:
        return BERT_PRETRAINED_CONFIG_ARCHIVE_MAP.keys()
